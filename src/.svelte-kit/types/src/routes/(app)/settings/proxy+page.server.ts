...
export const actions = {
	upload_picture: async ({ request, locals }: import('./$types').RequestEvent) => {
		try {
	...
	delete_picture: async ({ locals }: import('./$types').RequestEvent) => {
		try {
	...
	update_general_details: async ({ request, locals }: import('./$types').RequestEvent) => {
		try {
			const data = await request.formData();
	...
	update_password: async ({ request, locals }: import('./$types').RequestEvent) => {
		try {
			const data = await request.formData();
	...
	get_user_details: async ({ locals }: import('./$types').RequestEvent) => {
		if (locals && locals.user) {
