...
export const actions = {
	uploadMat: async ({ request, locals, params }: import('./$types').RequestEvent) => {
	...
	deleteMat: async ({ request, locals }: import('./$types').RequestEvent) => {
		validateLecturer(locals);
		try {
			const data = await request.formData();
	...
	submitObjects: async ({ request }: import('./$types').RequestEvent) => {
		const data = await request.formData();
