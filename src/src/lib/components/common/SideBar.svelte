<script lang="ts">
  ...
  function getClasses(href: string) {
    const baseClasses = "flex items-center rounded-lg px-4 py-3 text-gray-600 transition-all duration-300 hover:bg-green-50 hover:text-green-700 dark:text-gray-300 dark:hover:bg-green-900 dark:hover:text-green-100";
    const activeClasses = "bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100 font-medium";
...
<div class="flex flex-col min-h-screen">
  <aside
    class="flex m-4 w-64 flex-1 flex-col overflow-y-auto rounded-xl bg-white shadow-xl px-4 py-6 dark:bg-gray-900 transition-all duration-300"
  >
    <a href="/" class="mx-auto mb-8">
      <div class="flex items-center">
        <img class="mr-3 h-10 w-10" src="images/class-connect-logo.png" alt="ClassConnect owl logo" />
    ...
    <nav class="flex-1 space-y-1">
      {#each currentLinks as { icon, name, href }}
        <a
          ...
          {href}
        >
          <svelte:component this={icon} class="h-5 w-5 mr-3" />
    ...
    <a
      ...
      href="/signout"
    >
      <svelte:component this={ArrowLeftToBracketOutline} class="h-5 w-5 mr-3" />
