<script>
  ...
  import { Button, Tabs, TabItem, Table, TableBody, TableBodyCell, TableBodyRow, TableHead, TableHeadCell, TableSearch, Button, Dropdown, DropdownItem, Checkbox, ButtonGroup } from 'flowbite-svelte';
...
<Section name="advancedTable" classSection='bg-gray-50 dark:bg-gray-900 p-3 sm:p-5'>
    ...
    <div slot="header" class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
      <Button>
        <PlusOutline class="h-3.5 w-3.5 mr-2" />Schedule Lesson
      </Button>
      ...
      <TableHead>
        <TableHeadCell padding="px-4 py-3" scope="col">Topic</TableHeadCell>
        <TableHeadCell padding="px-4 py-3" scope="col">Date</TableHeadCell>
        <TableHeadCell padding="px-4 py-3" scope="col">Starting Time</TableHeadCell>
        <TableHeadCell padding="px-4 py-3" scope="col">Ending Time</TableHeadCell>
      </TableHead>
      <TableBody class="divide-y">
        {#if searchTerm !== ''}
          {#each filteredItems as item (item.id)}
            <TableBodyRow>
              <TableBodyCell tdClass="px-4 py-3">{item.}</TableBodyCell>
