...
import Sidebar from '$lib/components/utils/student/+ModuleSideBar.svelte';

vi.mock('$app/stores', () => ({
	page: {
		subscribe: vi.fn().mockImplementation((fn) => {
			fn({ url: { pathname: '/student/module' } });
			return () => {};
		})
	}
}));
...
describe('Sidebar Component', () => {
	...
	it('renders navigation links with correct names and icons', () => {
		...
		const links = ['Announcements', 'Study Materials', 'SandBox', 'Classroom'];
	...
	it('applies the active class to the current page link', () => {
		...
		expect(activeLink).toHaveAttribute('href', '/student/module');
	});
});
