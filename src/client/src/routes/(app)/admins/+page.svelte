<script lang="ts">
  ...
  $: ({ admins } = data);
  let searchTerm = '';
  $: filteredAdmins = admins.filter((admin: { name: string; }) => 
    admin.name.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1
  );
...
<div class="container mx-auto px-4 py-8">
	{#if admins.length === 0}
  <Card img="https://www.coursesonline.co.uk/wp-content/uploads/Subject-Business-Administration.jpeg?height=485&dpr=2" size="none">
    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Boost Management By Adding Additional Administrators!</h5>
    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400 leading-tight">Add new administrators to manage the organisation.</p>
    <Button>
      Add Your First Administrator <ArrowRightOutline class="w-6 h-6 ms-2 text-white" />
    </Button>
  </Card>
	{:else}
					...
					<TableBodyRow>
						<TableBodyCell>
							<div class="flex items-center">
								<img src={admin.image} alt={`${admin.name}`} class="mr-2 sm:mr-4 h-8 w-8 sm:h-10 sm:w-10 rounded-full" />
								<div>
									<div class="font-semibold text-sm sm:text-base">{admin.name}</div>
									<div class="text-xs sm:text-sm text-gray-500">{admin.email}</div>
								</div>
							</div>
						</TableBodyCell>
						<TableBodyCell>
							<div class="flex space-x-2">
								<Button color="purple" on:click={() => handleEditModalOpen(admin.id)} class="p-1.5 sm:p-2">
									<EditOutline class="w-4 h-4 sm:w-5 sm:h-5" />
  	              <span class="hidden sm:inline ml-2 text-sm">Edit</span>
								</Button>
								<Button color="red" on:click={() => handleRemoveModalOpen(admin.id)} class="p-1.5 sm:p-2">
